...
export function assembleGraph(context, session, infoNodeId, indicatedStatus, files, filterAssembly, filterAnnotation) {
     ...
     */
    function fileCssClassGen(file, active, indicated, addClasses) {
        return `pipeline-node-file${active ? ' active' : ''}${indicated ? ' indicated' : ''}${addClasses ? ` ${addClasses}` : ''}`;
    }
    ...
    Object.keys(matchingFiles).forEach((fileId) => {
        ...
        const fileCssClass = fileCssClassGen(file, infoNodeId === fileNodeId, file.status === indicatedStatus);
        const fileRef = file;
...
// Render the indicator control panel.
const IndicatorControls = (props) => {
    const { selectedStatus, clickHandler } = props;
    return (
        <div className="indicator-controls">
            {fileStatuses.map(status =>
                <IndicatorControlsButton key={status} status={status} selected={status === selectedStatus} indicatorClickHandler={clickHandler} />
            )}
        </div>
    );
};

IndicatorControls.propTypes = {
    selectedStatus: PropTypes.string.isRequired, // File status currently selected for indication, or '' if none
    clickHandler: PropTypes.func.isRequired, // Function to call when the selected indicator changes
};


// Render one indicator control button. This allows the binding of a specific button's status to a
// click within it.
class IndicatorControlsButton extends React.Component {
    constructor() {
        super();
    ...
    clickHandler() {
        this.props.indicatorClickHandler(this.props.status);
    }

    render() {
        const { status, selected } = this.props;
        const selectedClass = selected ? 'indicator-controls--selected' : '';
        return <button key={status} className={selectedClass} onClick={this.clickHandler}>{status}</button>;
    }
}

IndicatorControlsButton.propTypes = {
    status: PropTypes.string, // Indicator status this button controls
    selected: PropTypes.bool, // True if this button should be rendered selected
    indicatorClickHandler: PropTypes.func, // Function called to handle a click in this button
};
...
class FileGalleryRendererComponent extends React.Component {
    constructor() {
        ...
        this.state = {
            ...
            indicatedStatus: '', // Which file status shows as selected for indication on the graph/table
        };
        ...
        this.indicatorClickHandler = this.indicatorClickHandler.bind(this);
    ...
    indicatorClickHandler(status) {
        this.setState(prevState =>
            ({ indicatedStatus: prevState.indicatedStatus === status ? '' : status })
        );
    }
    ...
    render() {
        ...
        if (graphFiles && graphFiles.length && !hideGraph) {
            try {
                const { graph, graphedFiles } = assembleGraph(context, this.context.session, this.state.infoNodeId, this.state.indicatedStatus, graphFiles, selectedAssembly, selectedAnnotation);
                jsonGraph = graph;
        ...
        return (
            <Panel>
                ...
                <IndicatorControls selectedStatus={this.state.indicatedStatus} clickHandler={this.indicatorClickHandler} />
...
class FileGraph extends React.Component {
    ...
    render() {
        const { items, graph, selectedAssembly, selectedAnnotation, handleNodeClick, indicatedStatus } = this.props;
        const files = items;
        ...
        if (files && files.length) {
            ...
            if (goodGraph) {
                if (selectedAssembly || selectedAnnotation) {
                    return <Graph graph={graph} nodeClickHandler={handleNodeClick} nodeMouseenterHandler={this.handleHoverIn} nodeMouseleaveHandler={this.handleHoverOut} indicatedStatus={indicatedStatus} noDefaultClasses forceRedraw />;
                }
...
FileGraph.propTypes = {
    ...
    indicatedStatus: PropTypes.string, // File status to show as indicated in the graph
};
